"""Microduck forward-roll (roulade) task.

Episodic policy: the robot starts standing, rolls forward over the flat top of
...
Design (the roulade section of task_mdp.py has the details):
  • ONE dense progress signal — paid increments of the max-so-far cumulative
    forward rotation. Potential-based: a full roll pays 2π worth in total and
    camping anywhere pays zero per step.
  • Rotation only accumulates while the robot TOUCHES THE GROUND (a roulade
    never leaves the floor). Without this support gate the policy learned a
    ballistic "breakdance" whip — same 2π, sooner, at no cost.
  • Landing rewards gate on ROLL COMPLETION (rotation frontier ≥ ~260°), not
  ...
  • Reverse curriculum via mid-roll spawns: a slice of episodes starts partway
    into the roll, tucked, with forward angular momentum and the accumulator
    pre-set. The second half of a roulade IS the face-up recovery problem,
    which standup proved learnable.

DR / obs / regularisers mirror the standup env (velocity sim2real parity), with
the motion-blockers (body_ang_vel, |a_z|, arrival damping) kept near zero during
discovery and introduced late by curriculum — the roll IS a large
angular-velocity, large-impact event, and taxing attempts prevents discovery.
"""

import math
from copy import deepcopy

# The roll is sagittal / left-right symmetric, and the mirror loss directly
# fights the sideways-collapse failure mode.
...
# A controlled roll takes ~2 s + rise ~1.5 s + settle; 4 s left no room for the
# rise after a paced roll.
EPISODE_LENGTH_S = 5.0

# Measured standing trunk height — never guess this.
STAND_Z = 0.115
...
# leaning back; >260° opens the landing gate. The MAX must stay wide: with a
# 185° ceiling the second half of the roll (supine → seated → rise) was never
# spawned and never learned. Spawns past ~300° open the landing gate at birth,
...
# Tuck anchor: legs folded + CHIN TUCK. neck_pitch −1 / head_pitch +1 puts the
# flat head top squarely on the floor (measured axis_z −0.99, vs +0.6 for a
# passive face-plant); the head-top latch REQUIRES this, so mid-roll spawns
# must demonstrate the tucked configuration. Servo-index keyed; mid-roll spawns
...
def make_microduck_roulade_env_cfg(play: bool = False) -> ManagerBasedRlEnvCfg:
    ...
    # The roll's pivot signal. jaw_soft carries the head collision geoms
    # (top_head_shell = the flat top, jaw, bottom_head_shell). NAME IS
    # LOAD-BEARING: _update_roulade_accum reads it for the over-the-head latch.
    ...
    # The SUPPORT GATE: the rotation accumulator only integrates while some
    # robot geom touches the terrain, so ballistic flips earn no progress and
    # never complete. NAME IS LOAD-BEARING: _update_roulade_accum reads it.
    ...
    # The one dense task signal during the roll: during a 1.5 s roll it
    # averages ~0.7/step, so a full roll pays ≈ weight × 50.
    cfg.rewards["roulade_progress"] = RewardTermCfg(
        func=microduck_mdp.roulade_progress,
        weight=8.0,
        # max_paid_rate: the over-the-top transit is MEASURED at 3.5–5.5 rad/s.
        # This robot is 10 cm tall and its natural tumble timescale is fast; a
        # 3 rad/s cap forfeited most of the physically-necessary rotation.
        # Style pressure belongs in |a_z| / action_rate / the support gate, not
        # in fighting gravity's clock.
        params={"target_angle": 2 * math.pi, "max_paid_rate": 5.0},
    )

    # Whip-speed tax. The threshold sits above the measured p90 transit speed
    # (~5.5 rad/s), so it taxes genuine whips, not the natural tumble.
    ...
    # Tight-std upright × height product on top of the broad composite:
    # without it every completed episode parked at the same z≈0.105 / 27°-lean
    # pose, where the broad stds already score ~0.5 and offer no gradient to
    # finish. The sharp layer is ~0.1 at that basin vs ~1.0 upright.
    ...
    # Once the rotation is done, every step spent below STAND_Z costs, so
    # "crumple in a heap after the roll" flips from free to net-negative (the
    # fix that broke standup's static-sit basin). The gate is closed during the
    ...
    # Upward CoM velocity, gated to the late-roll region: supine → up is the
    # face-up-recovery problem, where end-state rewards have zero gradient at
    # zero motion.
    ...
    # Straightness. Rolling over the SHOULDER is a lower-energy path than
    # straight over the head (it avoids the fully-inverted configuration), and
    # the policy will take it. The structural fix is the flatness gate on the
    # accumulator + the head-top latch — side rolls then don't count as
    # rotation at all; these penalties add the dense per-step gradient back
    # toward the plane, and must not be noise against progress@8.
    ...
    # |a_z| impact shaping, active from step 0: discovery is easy in this env,
    # so style is the scarce resource, not exploration. Under zero impact cost
    # the policy locks in a violent solution. Curriculum ramps it further.
    # ⚠️ trunk_vertical_accel_penalty is SELF-NEGATING (returns -|a_z|) →
    # POSITIVE weight; a negative weight here would reward violence.
    ...
    # An always-on upright reward would oppose the flip; landing uprightness is
    # handled by the completion-gated terms above.
    ...
    # Zero padding for BOTH head (4) and body (6): the head is part of the task
    # (it's the pivot), so there is no head_pose command here, but the 61D obs
    # layout parity is kept so the runtime stack works unchanged (send zeros).
    ...
    # Standing + mid-roll spawns; also resets the rotation accumulator. MUST
    # run after reset_robot_joints (dict insertion order): the mid-roll tuck
    # lerps FROM the HOME pose that event writes.
    ...
    # Heavy mid-roll early — the completion sub-task is learnable from day 0
    # (it overlaps face-up recovery) — then shift toward standing starts as the
    # full roll gets discovered. Mid-roll never goes to zero: it keeps the
    # second half practiced and is realistic DR anyway. Do not shift away from
    # mid-roll before standing-spawn rolls are mastered; an earlier schedule
    # left progress at ~20% of a full roll.
    ...
    # Landing metrics declined in lockstep with the -0.4/-0.6 stages of an
    # earlier, tighter ramp — the tightening was squeezing the rise. The -0.1
    # minimum from step 0 stays: near-zero smoothing bred violence.
    ...
    # Introduced only after the roll skill exists: any attempt-tax active
    # during discovery prevents the maneuver from being found at all. The fix
    # is timing, not magnitude.
