...
The robot spawns on flat ground, rolls onto a descending ramp and lets itself
glide down while staying upright. No steering: the twist command is neutralized
(rel_standing_envs=1.0). Custom flat+ramp terrain (FlatRampTerrainCfg) with a
steepness curriculum (terrain_levels_slope). DR/obs/reset are inherited as-is
from make_microduck_velocity_rollers_env_cfg → 61D obs, runtime-interchangeable.
"""
...
# Steepness at PLAY: None = random (as in training), or 0..1 to force a precise
# slope (1.0 = steepest ~20°). Overridable via the SLOPE_PLAY_DIFFICULTY env var.
...
# Below the lowest exit flat (steepest and longest ramp) plus margin, so it
# never triggers during a normal descent — only if the robot leaves solid ground.
...
def make_microduck_roller_slope_env_cfg(play: bool = False) -> ManagerBasedRlEnvCfg:
    ...
    # At play, show varied slopes: None → random steepness across all rows.
    if play:
    ...
    # The inherited yaw is random, so fix it to 0 (facing the bottom). No base
    # velocity is injected: the robot spawns on the ramp and gravity makes the
    # wheels roll. The old base push (fast base, stationary wheels) skidded →
    # contact spike → NaN divergence, and the robot "walked to stop" instead of
    # rolling.
    cfg.events["reset_base"].params["pose_range"]["yaw"] = SPAWN_YAW
    # Initial momentum is given as consistent ROLLING (base + wheels, ω·r = v)
    # by reset_rolling_entry below.
    cfg.events["reset_base"].params["velocity_range"] = {}

    # === REWARDS: FREE balance (it places its center of gravity itself) ===
    # NO fixed-pose reward: dictating the flat-ground standing posture prevented
    # it from flexing/leaning. It is free to move its CoM (hips/knees, lean) to
    # hold the slope. We reward staying upright, alive, gliding and straight.
    keep = {"action_rate_l2"}
    ...
    # Rewards the wheels ROLLING downhill, capped: no incentive to push faster,
    # and "running" (pushing the base without rolling) pays nothing. Without a
    # glide reward the optimum would be to stay still.
    cfg.rewards["wheel_glide"] = RewardTermCfg(func=microduck_mdp.wheel_glide_reward, weight=2.0, params={"cap_speed": 0.35})
    # Hold the spawn yaw (0 = facing the descent). Corrective, so the robot can
    # recover. NB: SYMMETRY_CFG is coded for the old 51D obs → unusable here.
    ...
    # The fixed LEG pose was removed for free balance, which left nothing
    # holding the head — it went anywhere. This constrains head/neck only.
    ...
    # The exit flat gives solid ground at the bottom, so no edge termination is
    # needed (terrain_edge_reached cut long ramps too early).
    ...
    # === OBS: sanitize NaN/Inf (rare contact divergences) ===
    # A rare contact (~1/25M env-steps) diverges the free-joint to NaN. Because
    # of the one-substep offset, nan_state only catches it at the NEXT step, but
    # the NaN already reached this step's obs → rsl_rl's check_nan kills the
    # training. "sanitize" replaces NaN/Inf with 0 in the returned obs;
    ...
    # Starts at 2° and promotes toward 20° once the robot has descended far
    # enough (terrain_levels_slope, distance-based). Viable only because the
    # glide reward makes it MOVE FORWARD — a still robot never promotes.
