...
# geoms, STAND pose, trunk brought down to contact) on scene_rollers.xml:
# standing 0.1407, resting face down 0.0752, resting face up 0.0475.
# Check: the model WITHOUT wheels gives 0.1172 kinematically vs STAND_Z=0.115
# measured under load by standup → ~2 mm of sag, applied here too.
# 0.138 falls within the reset_base z (0.1335–0.1435) already used by the roller env.
...
# At play, the env is rebuilt from scratch: common_step_counter restarts at 0, so the
# ground_state_mix curriculum applies its stage 0, where face_up_prob = 0. We therefore
# NEVER see a face-up start at play — yet it is the hardest case,
# the one we want to inspect by eye. This variable forces it.
...
# Only has an effect with play=True. Same pattern as SLOPE_PLAY_DIFFICULTY in
# roller_slope.
PLAY_FACE_UP = None
# Face-down:standing ratio of the LAST curriculum stage (0.40 / 0.20 = 2:1). The
# remainder (1 - face_up) is split in this ratio, so that 0.4 reproduces
# exactly the end-of-training mix.
...
# The standup uses [0-4, 9-13] / [5-8]: these are the indices of the model WITHOUT
# wheels, they do NOT hold here. Locked by tests/test_roller_standup_cfg.py.
#
...
def make_microduck_roller_standup_env_cfg(play: bool = False) -> ManagerBasedRlEnvCfg:
    ...
    # run vweolw91. This is the cause of the "very violent", and it also explains the
    # unsuccessful damping attempts documented in the standup, which
    # were fighting a term actively pushing the other way.
    #
    # We keep the 0.02 magnitude (the one originally intended) DELIBERATELY small:
    ...
    # The standup identified it as the only damper that does not kill the
    # rise from the back, so it is THE safe lever to raise.
    #
    # -2e-3 (value inherited from standup) only contributed -0.0002/step against
    # ~+41.6 of task reward saturated at 95-99 % — i.e. nothing at all. All
    # dampers combined the ratio was ~35:1 in favor of the task, so
    # no reason to be gentle. Measured on run vweolw91 at iteration 7500.
    #
    # Recalibration: the raw value of |Δτ|² is ~0.1 at convergence, so
    # contribution ≈ 0.1 × |weight|. Measured at -0.255/step with a weight of -2.0 (run
    # d8rnko6p) — so NOT the cause of the freeze, but we go back down to -0.2 to free
    # the damping budget while isolating the effect of the sign bug alone.
    ...
    # converged to staying lying down, INERT. Measured (run d8rnko6p):
    # head_impact_penalty -1.01/step, the largest negative term in the table, while
    # standing_composite collapsed from +14.3 to +3.3.
    #
    # The reasoning error was to believe that a "targeted" penalty does not
    # restrain the motion. False here: to get up from the back, this robot PIVOTS on its
    # head and shoulders. The head is the fulcrum of the roll-over, not
    # collateral damage — penalizing it blocks the only available mechanism, and the
    # back was already the failing case.
    #
    # Hypothesis under test: hitting the head was a SYMPTOM of the violence
    # (the gentle_rise sign bug paid for brutality, and a brutal rise
    # ends on the head), not a separate defect. If the slam comes back once the sign
    # is fixed, the fix must be a HEIGHT-GATED penalty — as
    # upright_sharp is — to spare the roll-over phase on the ground.
    #
    # ⚠️ Beware of the lazy optimum that makes this freeze possible: pose_stand_legs
    # stayed at +7.72 out of 8 while the robot was lying down (legs at HOME in
    # lying position → reward collected almost for free). It is
    # height_stand_l1 (weight +30) that must make "staying on the ground" net negative.

    # ── Start ON THE GROUND: face down / face up / already standing ──────────
    # Added LAST in cfg.events: the execution order follows the insertion
    # order, and this term must overwrite the pose set by reset_base /
    # reset_robot_joints.
    # The "already standing" bucket is not decorative: without it the policy learns
    # to rise but not to HOLD, and it falls back right after getting up.
    # No "sitting" bucket → no sitting_joint_overrides to remap (those of the
    # standup are indices of the model WITHOUT wheels).
    ...
    cfg.events["set_ground_state"] = EventTermCfg(
        ...
        params={
            ...
            # from 0.0752, the back rests at 0.0475. A single floor therefore
            # cannot be ideal for both. We choose 0.076 to eliminate
            ...
            # Pitch/roll noise at start. Beware: in
            # set_random_ground_state the "standing" bucket reuses the quaternion
            # of the "sitting" bucket, so this noise ALSO applies to standing
            # starts — this is intended (no overfitting to perfectly straight).
            "sitting_tilt_max": math.radians(10),
        },
    )

    # The robot STARTS fallen → the tilt termination makes no sense here
    # (it would kill the episode at the first step). nan_state, inherited, stays.
    cfg.terminations.pop("fell_over", None)

    # Starting-pose curriculum, easy → hard. With a flat mix from the
    # start, the policy optimizes the easy majority and leaves the back under-trained
    ...
    # that, event_param_curriculum (which runs BEFORE the reset events) would
    # rewrite them with its stage 0 at the first reset. Play only, so
    # training and its easy → hard curriculum are untouched.
    if play:
    ...
    # This is the only truly new piece of this env, and the heart of the
    # difficulty: the wheels roll, so there is NO longitudinal grip
    # to push on the ground. The roller env RAISES this
    # friction (0 → 0.0015); here we LOWER it, to bootstrap the
    # gesture on an easy problem (nearly locked wheels ≈ feet) before
    # imposing the real physics of rolling.
    #
    # DIAGNOSTIC to watch: if Episode_Reward/standing_composite collapses at
    # a stage, the "grippy feet" gesture does not transfer to free wheels
    # → a skater technique will have to be guided (intermediate knee support,
    # one skate at a time). That is a usable result, not a failure.
    #
    # sim2real WARNING: only checkpoints from AFTER the last stage
    ...
    # practice. It just keeps the event's DEFAULT value consistent with
    # stage 0 of the curriculum, in case someone removes the curriculum later
    # while leaving the event in place.
    cfg.events["randomize_wheel_friction"].params["ranges"] = _WHEEL_FRICTION_STAGE0

    # ── action_rate: the standup ramp, not the roller one ────────────────────
    # The roller env goes up to -2.0 for a calm gait. That is a motion
    # blocker: it slows the fast action that the rise from the back
