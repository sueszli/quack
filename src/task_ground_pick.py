...
the ground WITHOUT touching it (mouth pointing down), then returns to a clean
standing pose, stable and robust to pushes. Obs/action spaces match the walking
policy so the two can be swapped at runtime.

Task-space objective (no DOWN pose): mouth_ground_proximity pulls the mouth
down, head_impact_penalty (strong) forbids contact → the equilibrium is the
mouth just above the ground; mouth_perpendicular_to_ground orients it downward.

Phase encoding (in the 3-D command slot):
...
Phase is randomised per env on reset to de-correlate environments and avoid
synchronised oscillations.

The task-specific REGULARIZATION is deliberately HEAVIER than velocity's: slow
careful reaching wants more damping than walking.
"""

import math
from copy import deepcopy

# Symmetry OFF: SYMMETRY_CFG's _OBS_PERM is hardcoded for the old 51D obs
# layout and breaks on the 61D obs.
...
ENABLE_NECK_OFFSET_RANDOMIZATION = False  # the head is used for the task

# ── Ranges (matched to the velocity env) ──────────────────────────────────────
COM_RANDOMIZATION_RANGE = 0.003  # ramped by curriculum
HEAD_COM_RANDOMIZATION_RANGE = 0.003  # ramped by curriculum
...
VELOCITY_PUSH_RANGE = (-0.15, 0.15)  # quasi-static gesture; ±0.3 made it fall even when upright
...
# Rewards are gated by a 4-segment profile rather than sinusoidal weighting
# (which couples descent/hold/rise): SLOW descent and rise, SHORT low hold,
# long standing rest. At GP_PERIOD = 4 s: descent 1.5 s, low hold 0.2 s, rise
# 1.5 s, rest 0.8 s.
# ⚠️ RISE_END=0.80 exceeds the φ=0.7 cutoff of the infer script: the rise only
# completes if the slot plays out to φ~1.0. Check the actual runtime window.
# ⚠️ --ground-pick-period at deployment must equal GP_PERIOD.
GP_PERIOD = 4.0
DESCENT_END = 0.375
HOLD_END = 0.425
RISE_END = 0.80
...
def make_microduck_ground_pick_env_cfg(play: bool = False, rough: bool = False) -> ManagerBasedRlEnvCfg:
    ...
    # Covers the neck subtree. Used by head_impact_penalty to stop the policy
    # slamming the head into the ground during the approach.
    ...
    # target_height=0 pulls the mouth to the ground; std=0.10 gives gradient
    # from ~20 cm (the standing stance). "WITHOUT TOUCHING" comes from the
    # strong head_impact_penalty below — the equilibrium is mouth just above.
    ...
    # alignment ∈ [-1, 1]: 1 = mouth x-axis vertical, 0 = horizontal, -1 = up.
    ...
    # Return phase — legs. The passive jaw joints are not part of the
    # articulation, so joint_pos is the clean 14-joint layout.
    ...
    # head-body collision: the head geoms have no collision mesh, so
    # self_collisions cannot catch it and this pose reward is the only guard.
    ...
    # Vertical trunk rewarded ONLY during the rise: the pose return alone does
    # not guarantee dynamic balance while extending. Gated on the return so it
    # does NOT hinder the forward lean of the approach.
    ...
    # Anti-nosedive: penalizes neck velocity during descent+hold (gate=0 on the
    # rise, so standing up is not restrained).
    ...
    # Weight-0 reward used as a per-step hook: applies the payload's WEIGHT as
    # an external force at mouth_tip, gated on the rise. The payload is drawn
    # at reset by sample_mouth_payload.
    ...
    # Reduced weight — the robot needs to lean forward during the approach.
    ...
    # Keep BOTH feet in contact. This is CONTACT only; a foot tipping over the
    # ankle is handled by feet_flat below.
    ...
    # feet_grounded only sees CONTACT, so a foot that PIVOTS on the ankle while
    # keeping one contact point slips through. feet_flat_penalty projects
    # gravity into the foot site frame: flat → site Z vertical (xy²≈0), any
    # tipping → xy²>0.
    ...
    # ── Rewards: regularisation (HEAVIER than velocity) ──────────────────────
    # The ground-pick motion is slow and precise, so strong smoothness aids
    # transfer — unlike the dynamic standup recovery, where heavy
    # regularisation blocked the motion.

    # Ramped in by the curriculum below, ending at -2.0 vs velocity's -1.0.
    cfg.rewards["action_rate_l2"] = RewardTermCfg(func=mdp.action_rate_l2, weight=-2.0)

    # Higher weight because the head is heavily used.
    cfg.rewards["neck_action_rate_l2"] = RewardTermCfg(func=microduck_mdp.neck_action_rate_l2, weight=-1.0)

    # Penalises fast/forceful moves.
    cfg.rewards["joint_torques_l2"] = RewardTermCfg(func=microduck_mdp.joint_torques_l2, weight=-5e-3)

    # Head and neck could clip the legs during a deep crouch.
    ...
    # No-touch enforcement: strong penalty, low threshold, so any ground contact
    # is expensive. Against mouth_ground_proximity this sets the equilibrium at
    ...
    # No terrain-height sensor in this env.
    ...
    # policy reads is ~1 control period old.
    ...
    # encoder-bias `biased` flag below must not leak into the critic. The
    # passive-exclusion regex is a no-op here (no passive joints) but kept for
    # parity with the other envs.
    ...
    # ── Pad the command vector to the unified 13D layout ──────────────────────
    # Ground-pick drives the head from the task phase, not a command, but all
    # microduck policies share the 61D obs shape so the runtime can feed a
    # single command buffer. The trailing 10 slots stay zero.
    ...
    # The segmented profile (constants at the top of the file) decouples
    # descent/hold/rise/rest.
    ...
    # ── Terminations ──────────────────────────────────────────────────────────
    # Terminate on NaN physics (extreme contact impulses) before it hits the obs.
    ...
    # Random payload "in the mouth", drawn per episode and applied on the rise
    # by the mouth_payload_force hook.
    ...
    if ENABLE_VELOCITY_PUSHES:
        # Play uses a spaced-out interval to judge the gesture on realistic
        # behavior; 0.5-1 s was a stress test that made it fall even when upright.
    ...
    if ENABLE_KP_RANDOMIZATION or ENABLE_KD_RANDOMIZATION:
        # Dormant. randomize_delayed_actuator_gains predates canonical BAM —
        # only enable after porting it to BamActuator.set_gains.
    ...
    # Warm up light so the gross reaching motion can form, then clamp down hard.
