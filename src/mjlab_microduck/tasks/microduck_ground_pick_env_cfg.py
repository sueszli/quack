...
ENABLE_NECK_OFFSET_RANDOMIZATION     = False  # head is used for the task
...
VELOCITY_PUSH_RANGE              = (-0.15, 0.15)  # quasi-static motion; ±0.3 toppled it even upright
...
# Segmented phase profile at GP_PERIOD 4 s: descent 1.5 s, low hold 0.2 s, rise 1.5 s,
# standing rest 0.8 s. RISE_END 0.80 means the rise only completes if the runtime plays the
# slot to φ ≈ 1.0; deploy with --ground-pick-period 4.0.
GP_PERIOD    = 4.0
DESCENT_END  = 0.375
HOLD_END     = 0.425
RISE_END     = 0.80
...
def make_microduck_ground_pick_env_cfg(play: bool = False, rough: bool = False) -> ManagerBasedRlEnvCfg:
    ...
    # Pulls the mouth to the ground (target 0); "without touching" comes from
    # head_impact_penalty below, so the equilibrium is the mouth just above the floor.
    ...
    # Tight std: head geoms have no collision mesh, so this is the only guard against
    # backward overshoot into the body.
    ...
    # Upright only during the rise, so the forward lean of the approach is untaxed.
    ...
    # Brakes the head dive during descent/hold only.
    ...
    # Weight-0 per-step hook applying the sampled mouth payload (10-40 g) as an external
    # force during the rise.
    ...
    # Light: the robot must lean forward during the approach.
    ...
    # feet_grounded only sees contact; a foot pivoting on its edge passes. This projects
    # gravity into the foot frame and penalises any tilt.
    ...
    # Regularisation heavier than velocity: slow precise reaching benefits from smoothness
    # (unlike dynamic recovery, where it blocks the motion).
    ...
    # Counterpart of mouth_ground_proximity: any head-ground contact is expensive.
    ...
    # Cyclic phase in the twist slot.
    ...
    # Per-episode mouth payload (10-40 g), applied during the rise by mouth_payload_force.
    ...
    if ENABLE_KP_RANDOMIZATION or ENABLE_KD_RANDOMIZATION:
        # randomize_delayed_actuator_gains predates BAM; port to BamActuator.set_gains first.
    ...
    # Light while the reaching motion forms, then -2.0 (heavier than velocity).
