"""Microduck skating stroke environment.

Episodic motion primitive: robot executes one push-left / push-right skating
cycle to gain forward speed, then the episode ends.  Deployed by triggering
the policy for a fixed number of steps on button press; between presses the
robot coasts or balances with a separate policy.

Reward is purely forward speed (linear, not Gaussian) plus regularisation.
Short episodes (3 s) mean walking cannot compete with a good skating stroke.
"""
...
def make_microduck_skating_stroke_env_cfg(
    play: bool = False,
) -> ManagerBasedRlEnvCfg:
    """Skating stroke motion primitive environment."""
    ...
    # Loose hip_roll — skating stroke requires wide lateral push
    std_moving = {
        r".*hip_yaw.*": 0.3,
        r".*hip_roll.*": 0.8,
        r".*hip_pitch.*": 0.4,
    ...
    # Short episodes — one push stroke cycle
    ...
    # Pose — mutate in place (function already configured by base env)
    cfg.rewards["pose"].params["std_standing"] = std_standing
    cfg.rewards["pose"].params["std_walking"] = std_moving
    cfg.rewards["pose"].params["std_running"] = std_moving
    ...
    # Core: linear forward speed — no Gaussian, no saturation
    # Walking in 3 s cannot compete with a good skating stroke
    ...
    # === COMMANDS ===
    # Fixed forward command — robot always pushes in the same direction
    command: UniformVelocityCommandCfg = cfg.commands["twist"]
    command.rel_standing_envs = 0.0
    command.rel_heading_envs = 0.0
    command.ranges.lin_vel_x = (0.5, 0.5)  # fixed, just used as direction signal
...
MicroduckSkatingStrokeRlCfg = RslRlOnPolicyRunnerCfg(
    ...
    algorithm=RslRlPpoAlgorithmCfg(
        ...
        clip_param=0.2,
        entropy_coef=0.05,  # higher entropy: explore different stroke mechanics
    ...
    experiment_name="skating_stroke",
    run_name="skating_stroke",
    save_interval=250,
