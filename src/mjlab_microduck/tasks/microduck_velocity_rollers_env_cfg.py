...
VELOCITY_PUSH_RANGE              = (-0.2, 0.2)  # gentler than walk
...
def make_microduck_velocity_rollers_env_cfg(
    ...
    # Each ankle subtree's only collision geoms are its two tires: per-foot semantics.
    ...
    # No env-side action clip: the runtime doesn't clip, so it would be a train/deploy
    # mismatch. Over-commanding is deterred policy-side (action_over_limit).
    ...
    # Stance foot only: ungated, this was minimised by keeping both blades down (swizzle).
    ...
    # Penalise the COMMAND beyond limit + 0.3 (the ±10 rad ctrlrange let hip_roll slam its
    # ±0.38 stop at max torque); a qpos penalty stole reachable range and broke the gait.
    ...
    # L1 closes the splayed stance at rest; stride rewards overpower it during a push.
    # Raise if still splayed, lower if it flattens the stride.
    ...
    # vel_scale 0.3 saturates the tanh near the achievable ~0.33 m/s; 0.5 kept pushing
    # faster than possible and destabilised the launch.
    ...
    # Velocity-gated (an in-place flutter farmed it otherwise). air_time drives swing
    # frequency, glide drives commitment; weights favour glide. A [0.40, 1.00] window
    # forced long violent kicks that tipped the real robot.
    ...
    # Coasting on ONE blade with quiet legs.
    ...
    # Core anti-swizzle signal: exactly one blade down while pushing, both down penalised.
    # (A continuous recover-to-neutral-pose reward re-opened the swizzle; any retry must
    # be phase-gated and keep the double-support penalty.)
    ...
    # Cumulative swing-time imbalance |L-R|/(L+R): a one-legged stride veers at launch.
    cfg.rewards["gait_symmetry"] = RewardTermCfg(
        func=microduck_mdp.gait_symmetry_penalty,
        weight=-1.0,
        params={"sensor_name": "feet_ground_contact"},
    )
    # (contact_frequency was tried to slow the cadence: minimised by never lifting a foot,
    # i.e. the swizzle. The air-time window is the safe cadence lever.)
    ...
    # Heading command is off; hold the yaw ANGLE near spawn (a yaw-rate penalty froze the
    # yaw and made drift worse).
    ...
    # XML keeps wheel frictionloss=0 for trainability; the curriculum ramps in bearing drag.
    ...
    if ENABLE_ARMATURE_RANDOMIZATION:
        # Legs/head only; wheel bearings are covered by the frictionloss event.
    ...
    # Servo joints only (14-dim obs). Deepcopy so `biased` below hits the actor only.
    ...
    # ang_vel_z is the clip limit for cmd[2] = heading error; 0 = no turning demand.
    ...
    # Main "calmer gait" lever; dial back if it can't push enough to move.
    ...
    if ENABLE_WHEEL_FRICTION_RANDOMIZATION:
        # Drag added at iter 750 (as wheel_speed peaked) pushed the policy into a
        # heading-farming optimum; keep wheels free until skating is robust.
    ...
    # CoM DR capped lower than the walker: skates are less forgiving.
    if ENABLE_COM_RANDOMIZATION:
