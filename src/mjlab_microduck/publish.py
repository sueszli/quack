...
def _resolve_weights(cfg: PublishConfig, workdir: Path) -> tuple[Path, dict]:
    ...
    training: dict = {"repo": "pollen-robotics/microduck_rl", **git_provenance()}
...
def run(cfg: PublishConfig) -> int:
    ...
    try:
        ...
        staged = workdir / "repo"
        staged.mkdir()
        shutil.copyfile(onnx_path, staged / POLICY_FILE)
        (staged / "manifest.json").write_text(dump_manifest(manifest))
        (staged / "README.md").write_text(render_readme(manifest, cfg.repo))
        ...
        first = install_commands(manifest, cfg.repo).splitlines()[0]
