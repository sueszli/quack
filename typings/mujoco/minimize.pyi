...
class Norm(abc.ABC):
  ...
  * `grad_hess(self, r: np.ndarray, proj: np.ndarray)`: Computes and returns
  ...
  @abc.abstractmethod
  def grad_hess(self, r: np.ndarray, proj: np.ndarray): # -> None:
...
class Quadratic(Norm):
  ...
  def value(self, r: np.ndarray): # -> Any:
    """Returns the quadratic norm of `r`."""
    ...
  
  def grad_hess(self, r: np.ndarray, proj: np.ndarray): # -> tuple[NDArray[Any], NDArray[Any]]:
...
def least_squares(x0: np.ndarray, residual: Callable[[np.ndarray], np.ndarray], bounds: Optional[Sequence[np.ndarray]] = ..., jacobian: Optional[Callable[[np.ndarray, np.ndarray], np.ndarray]] = ..., norm: Norm = ..., eps: float = ..., mu_min: float = ..., mu_max: float = ..., mu_factor: float = ..., xtol: float = ..., gtol: float = ..., max_iter: int = ..., verbose: Union[Verbosity, int] = ..., output: Optional[TextIO] = ..., iter_callback: Optional[Callable[[List[IterLog]], None]] = ..., check_derivatives: bool = ..., x_scale: Optional[Union[float, np.ndarray, str]] = ...) -> Tuple[np.ndarray, List[IterLog]]:
  """Nonlinear Least Squares minimization with box bounds.
...
def jacobian_fd(residual: Callable[[np.ndarray], np.ndarray], x: np.ndarray, r: np.ndarray, eps: np.float64, n_res: int, bounds: Optional[List[np.ndarray]] = ...) -> Tuple[np.ndarray, int]:
...
def check_jacobian(residual: Callable[[np.ndarray], np.ndarray], x: np.ndarray, r: np.ndarray, jac: np.ndarray, eps: np.float64, n_res: int, bounds: Optional[List[np.ndarray]] = ..., output: Optional[TextIO] = ..., name: Optional[str] = ...) -> int:
...
def check_norm(r: np.ndarray, norm: Norm, eps: np.float64, output: Optional[TextIO] = ...): # -> None:
