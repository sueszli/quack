...
Instantaneous posture precision is an unescapable tax on motion, and any head
cost active during the ground phase blocks the head-pivot flip. So this term must
(a) charge only the DC bias, (b) accumulate NOTHING while fallen, and (c) start
from ~zero on arrival upright.
"""
...
class _Env:
    def __init__(self, n):
        ...
        # The neck-id cache is normally built from the real asset.
...
def test_prone_accumulates_nothing():
    ...
    env.scene._asset.data.joint_pos[:, :4] = 0.5  # 28° head "error"
...
def test_ungated_matches_velocity_env_behavior():
    # Without gate params: plain EMA of the raw error, so the pose is irrelevant.
    env = _Env(2)
    _set_pose(env, z=0.05, pitch_deg=90.0)
    env.scene._asset.data.joint_pos[:, :4] = math.radians(15)
    out = _run(env, 300)
...
def test_reset_clears_the_ema():
    ...
    out = _run(env, 1, **GATE)
    # One step after reset the EMA holds alpha*err (~0.005); env 1 still carries
    # the settled bias (~0.26).
...
def test_standup_cfg_wiring():
    ...
    # Gates match arrival_damping so "standing" means one thing.
...
def test_velstand_inherited_term_is_gated():
    # Velstand episodes survive falls, so the inherited term must not charge the
    # ground phase.
