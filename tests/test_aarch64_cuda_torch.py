...
def test_torch_is_a_direct_dependency():
    ...
    assert any(d.split("=")[0].split("[")[0].strip() == "torch" for d in deps), "torch must stay in [project.dependencies]: uv applies [tool.uv.sources] to DIRECT dependencies only. Removing it silently drops aarch64 back onto PyPI's CPU-only wheel."


def test_torch_source_is_pinned_to_a_cuda_index_on_aarch64():
    pyproject = tomllib.loads((_ROOT / "pyproject.toml").read_text())
    uv_cfg = pyproject["tool"]["uv"]
    assert "torch" in uv_cfg.get("sources", {}), "[tool.uv.sources] no longer has a torch entry -> aarch64 falls back to PyPI's CPU wheel and `train` dies with IndexError in select_gpus()."
...
def test_lockfile_routes_aarch64_torch_to_cuda_wheels():
    ...
    assert _registry(aarch64).startswith(_CUDA_INDEX), f"torch on aarch64 comes from {_registry(aarch64)!r} — a CPU wheel. Re-run `uv lock` after checking [tool.uv.sources]."
...
def test_x86_64_resolution_stays_on_pypi():
    ...
    for pkg in others:
        assert _registry(pkg) == "https://pypi.org/simple", f"x86_64 torch moved to {_registry(pkg)!r} — x86_64 would switch wheels."
