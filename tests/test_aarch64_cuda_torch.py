...
def test_torch_is_a_direct_dependency():
    ...
    assert any(d.split("=")[0].split("[")[0].strip() == "torch" for d in deps), (
        "torch must stay in [project.dependencies]: uv applies [tool.uv.sources] to DIRECT dependencies only. Removing it silently drops aarch64 back onto PyPI's CPU-only wheel."
    )
...
def test_torch_source_is_pinned_to_a_cuda_index_on_aarch64():
    ...
    assert "torch" in uv_cfg.get("sources", {}), (
        "[tool.uv.sources] no longer has a torch entry -> aarch64 falls back to PyPI's CPU wheel and `train` dies with IndexError in select_gpus()."
    )
...
def test_lockfile_routes_aarch64_torch_to_cuda_wheels():
    ...
    assert _registry(aarch64).startswith(_CUDA_INDEX), (
        f"torch on aarch64 comes from {_registry(aarch64)!r} — a CPU wheel. Re-run `uv lock` after checking [tool.uv.sources]."
    )
...
def _on_spark():
    return (
        ...
        and subprocess.run(["nvidia-smi"], capture_output=True, check=False).returncode == 0
    )
...
def test_installed_torch_actually_sees_the_gpu():
    ...
    assert torch.cuda.device_count() > 0, (
        f"torch {torch.__version__} (cuda={torch.version.cuda}) sees no GPU although nvidia-smi reports one -> select_gpus() will raise IndexError."
    )
