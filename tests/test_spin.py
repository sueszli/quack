...
def test_spin_rate_integral_matches_trapezoid_shape_at_rate_max_6():
    # Shape only, at a hypothetical rate_max=6.0: envelope 12.6 rad vs 4*pi
    # 12.566, hence the 1% tolerance. The SHIPPED target is the next test.
    n = 100_000
...
def test_spin_rate_max_integrates_to_2_1_times_itself_per_cycle():
    # The envelope area is 2.1 * rate_max rad per cycle (0.25 + 1.6 + 0.25),
    # whatever rate_max. Fails loudly if the target changes without someone
    # working out how many turns it now implies.
    n = 100_000
    phase = (torch.arange(n, dtype=torch.float64) + 0.5) / n
...
def test_spin_gate_is_zero_over_the_whole_rest_segment():
    # No primer may push toward the scissor during rest: that is what gives a
    # clean trick exit toward the roller policy.
    phase = torch.linspace(0.650, 0.999, 50)
    gate = mdp.spin_gate_by_phase(phase, **_ENV)
    assert torch.allclose(gate, torch.zeros_like(gate), atol=1e-6)


# Fake env: exercises the reward wrappers without MuJoCo.
class _FakeData:
...
def _phase_cmd(phases):
    """The twist slot as the policy sees it: [cos(2*pi*phi), sin(...), 0]."""
...
def test_spin_rate_track_uses_yaw_and_phase():
    # phase 0.30 = full rate. Motionless scores exp(-(3/1.5)^2) = 0.018, i.e.
    # std=1.5 stays calibrated to SPIN_RATE_MAX.
...
def test_spin_rate_track_penalizes_wrong_direction():
    # Clockwise when counter-clockwise is asked must score below motionless.
    ang = torch.tensor([[0.0, 0.0, -mdp.SPIN_RATE_MAX], [0.0, 0.0, 0.0]])
    env = _FakeEnv(_FakeEntity(_FakeData(ang_vel_b=ang)), cmd=_phase_cmd([0.30, 0.30]))
    r = mdp.spin_rate_track(env, std=1.5)
    assert r[0] < r[1]
...
def test_spin_stay_in_place_is_squared_planar_speed():
    ...
    # 0.3^2 + 0.4^2 = 0.25 ; z ignored
    assert torch.allclose(c, torch.tensor([0.0, 0.25]), atol=1e-6)


def test_spin_stay_in_place_is_attenuated_during_the_launch_ramp():
    # The discount is what keeps this term from opposing the injection of
    # angular momentum during the launch ramp.
...
def test_spin_stay_in_place_is_full_price_during_rest():
    # Unlike the primers, this term must NOT be gated off: rest means motionless.
...
def test_wheel_differential_rewards_counter_rolling_wheels():
    # Counter-clockwise: LEFT wheels roll backward, RIGHT forward.
    env = _wheel_env(
        [
            [-10.0, -10.0, 10.0, 10.0],  # good differential
            [10.0, 10.0, 10.0, 10.0],  # straight ahead
            [10.0, 10.0, -10.0, -10.0],  # clockwise
        ],
        [0.30, 0.30, 0.30],
    )
...
def test_wheel_differential_saturates():
    # tanh saturation is what stops a race for wheel speed.
...
def test_spin_grounded_rewards_both_blades_down_and_is_gated():
    # Rows: both blades down at rate, one, none, both down but at rest.
...
def test_leg_antisymmetry_prefers_scissor_over_mirror():
    # Mirror convention: q_L = -q_R is the SYMMETRIC pose, q_L = q_R the SCISSOR.
    env = _leg_env(
        [
            [0.4, 0.3, 0.4, 0.3],  # scissor
...
def test_neck_joint_pos_l2_pattern_can_exclude_head_yaw():
    ...
    pos = torch.tensor([[0.0, 0.0, 0.0, 1.0]])  # head_yaw off by 1 rad
    ...
    # The spin pattern excludes head_yaw, freeing it as a flywheel.
