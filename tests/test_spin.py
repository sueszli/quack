...
def test_spin_rate_segment_boundaries():
    # bounds of the 4 segments: 0 at the start, full rate over [accel_end, hold_end],
    # still full rate at the very start of braking, 0 from the rest segment on.
...
def test_spin_rate_accel_ramp_is_increasing():
    ...
    # middle of the launch ramp -> half the target
...
def test_spin_rate_brake_ramp_is_decreasing():
    ...
    w = mdp.spin_rate_by_phase(phase, **_ENV)
    assert torch.all(w[1:] <= w[:-1])
    # middle of braking -> half the target
    mid = mdp.spin_rate_by_phase(torch.tensor([0.5875]), **_ENV)
    assert torch.allclose(mid, torch.tensor([3.0]), atol=1e-6)


def test_spin_rate_integral_matches_trapezoid_shape_at_rate_max_6():
    # This test protects the SHAPE of the trapezoid (2.1 * rate_max rad per cycle),
    # not the target actually shipped: at rate_max=6.0 (hypothetical, cf. _ENV above)
    # it is ~4*pi rad = 2 turns. Exact envelope = 12.6 rad, 4*pi = 12.566 ->
    # 1 % tolerance. The target IN EFFECT is covered by the next test.
    n = 100_000
...
def test_spin_rate_max_integrates_to_2_1_times_itself_per_cycle():
    # THE test that protects the SHIPPED target (mdp.SPIN_RATE_MAX), as opposed to
...
def test_spin_gate_is_zero_over_the_whole_rest_segment():
    # during rest no primer may push toward the scissor -> zero gate,
    # which is what gives a clean trick exit toward the roller policy.
...
def test_spin_stay_in_place_is_attenuated_during_the_launch_ramp():
    # Same velocity, two phases: in the launch ramp (0.05 < accel_end) the
...
def test_spin_stay_in_place_is_full_price_during_rest():
    # During rest we want the robot MOTIONLESS: this term must NOT be switched off,
    # unlike the primers (spin_wheel_differential, spin_grounded, scissor).
...
def test_wheel_differential_rewards_counter_rolling_wheels():
    # counter-clockwise: LEFT wheels negative (skate goes backward), RIGHT
...
def test_wheel_differential_is_gated_off_during_rest():
    # same good differential, but in the rest phase -> zero gate -> not paid.
...
def test_leg_antisymmetry_prefers_scissor_over_mirror():
    # mirror convention: q_L = -q_R is a SYMMETRIC pose (bad here),
...
def test_leg_antisymmetry_is_gated_off_during_rest():
    # at rest the gate is zero: nothing pushes toward the scissor, free neutral stance.
