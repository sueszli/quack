...
# crouch_pose_blend segments: descent [0,0.1), low [0.1,0.5), rise [0.5,0.6),
# standing [0.6,1.0).
...
def test_reward_is_one_when_height_matches_target():
    # phase 0.5 = full plateau, so the target is height_low.
...
def test_reward_at_phase_zero_expects_high_stance():
    # phase 0 = standing, so the target is height_high.
