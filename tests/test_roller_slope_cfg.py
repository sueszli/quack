...
def test_rolling_entry_no_base_push():
    # Momentum must arrive as ROLLING: a base push with motionless wheels is a
    # skidding jolt.
    cfg = make_microduck_roller_slope_env_cfg()
...
def test_balance_rewards_no_fixed_pose():
    # No fixed pose: it must be free to move its CoM to hold the slope.
    cfg = make_microduck_roller_slope_env_cfg()
...
def test_has_wheel_glide_reward_not_base_speed():
    # Reward rolling, not base speed: base speed was farmed by running.
    cfg = make_microduck_roller_slope_env_cfg()
...
def test_no_roller_skating_rewards_survive():
    # heading_hold is deliberately re-added, hence absent from this list.
    cfg = make_microduck_roller_slope_env_cfg()
...
def test_spawn_yaw_faces_downhill():
    # Fixed at 0 (downhill is +x), not the inherited -pi/+pi.
    cfg = make_microduck_roller_slope_env_cfg()
...
def test_void_termination_present_no_edge_termination():
    ...
    # The runout flat replaced the terrain-edge terminations.
...
def test_obs_nan_policy_sanitize():
    # A rare contact NaN must not kill training.
    cfg = make_microduck_roller_slope_env_cfg()
