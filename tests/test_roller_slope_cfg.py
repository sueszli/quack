...
def test_rolling_entry_no_base_push():
    # momentum given as ROLLING (reset_rolling_entry), not as a base push
    # (base alone + motionless wheels = skidding jolt). So reset_base sets
    # no base velocity.
    cfg = make_microduck_roller_slope_env_cfg()
...
def test_balance_rewards_no_fixed_pose():
    # free balance: upright/alive/glide present, but NO fixed pose
    # imposed (it must be able to move its center of mass to hold the slope).
    cfg = make_microduck_roller_slope_env_cfg()
...
def test_has_wheel_glide_reward_not_base_speed():
    # "let itself glide" = roll (wheels), not reward base speed
    # (which it reached by running). wheel_glide present, descent_speed absent.
    cfg = make_microduck_roller_slope_env_cfg()
...
def test_no_roller_skating_rewards_survive():
    # the roller SKATING rewards must not survive (heading_hold is deliberately
...
def test_spawn_yaw_faces_downhill():
    # fixed yaw at 0: always facing the bottom of the slope (+x), not the inherited -pi/+pi
    cfg = make_microduck_roller_slope_env_cfg()
...
def test_void_termination_present_no_edge_termination():
    ...
    # no more "terrain edge" termination (replaced by the runout flat)
...
def test_curriculum_present_and_starts_gentle():
    # gentle->steep curriculum: starts on the gentlest ramp, promotion active
...
def test_terrain_tile_fits_geometry():
    # the tile must contain flat + max ramp + runout
    cfg = make_microduck_roller_slope_env_cfg()
    gen = cfg.scene.terrain.terrain_generator
