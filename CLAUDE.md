...
- `mjcf/` (repo root, `MJCF_DIR` in `microduck_constants.py`) — MJCF exports from Onshape
  (onshape-to-robot, one `config_mjcf_*.json` per model), scenes, meshes; `add_backlash.py` generates
  the backlash variants.
...
- Real deployments hot-swap ONNX policies (walk / stand / trick) with a shared
  obs contract — rehearse in `uv run infer` before touching the
